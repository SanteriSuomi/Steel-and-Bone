...
{
	#if !DISABLESTEAMWORKS
	public static event EventHandler OnSteamInitializedEvent;

	protected bool s_EverInitialized = false;
	...
	protected override void Awake()
	{
		...
		if (!m_bInitialized)
		{
			Debug.LogWarning("[Steamworks.NET] SteamAPI_Init() failed. Refer to Valve's documentation or the comment above this line for more information. " +
				"Ignore if steam is not open.", this);
			return;
		}

		DontDestroyOnLoad(gameObject);
		s_EverInitialized = true;
		OnSteamInitializedEvent?.Invoke(null, EventArgs.Empty);
		InitializeManagers();
	}

	private static void InitializeManagers()
	{
		StatsBase.InitializeStatsBase();
		SteamCallbacks.InitializeSteamCallbacks();
	}
